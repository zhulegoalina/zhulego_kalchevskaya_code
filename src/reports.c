...
void orders_by_driver_period() {
    ...
    snprintf(sql, sizeof(sql),
             ...
             "FROM orders o JOIN drivers d ON o.driver_id = d.id JOIN cars c ON o.car_id = c.id "
...
void car_total_stats() {
    ...
    snprintf(sql, sizeof(sql),
             ...
             "FROM orders o JOIN cars c ON o.car_id = c.id WHERE c.car_number = '%s';",
             car_number);
...
void driver_statistics() {
    ...
    const char *sql = "SELECT d.full_name, COUNT(o.id) as trips, SUM(o.cargo_weight) as total_cargo, SUM(o.driver_earnings) as total_earnings "
                      "FROM drivers d LEFT JOIN orders o ON d.id = o.driver_id GROUP BY d.id;";
    printf("\n=== Driver Statistics ===\n");
    execute_query(db, sql);
    db_close(db);
}
...
void driver_fewest_trips() {
    ...
    const char *sql = "SELECT d.full_name, d.category, d.experience, COUNT(o.id) as trips, SUM(o.driver_earnings) as earnings "
                      "FROM drivers d LEFT JOIN orders o ON d.id = o.driver_id GROUP BY d.id ORDER BY trips ASC LIMIT 1;";
...
void car_highest_mileage() {
    ...
    const char *sql = "SELECT c.car_number, c.brand, c.mileage, SUM(o.distance) as total_distance "
                      "FROM cars c LEFT JOIN orders o ON c.id = o.car_id GROUP BY c.id ORDER BY total_distance DESC LIMIT 1;";
...
void earnings_all_drivers_period() {
    ...
    snprintf(sql, sizeof(sql),
             ...
             "FROM drivers d JOIN orders o ON d.id = o.driver_id WHERE o.order_date BETWEEN '%s' AND '%s' GROUP BY d.id;",
             start_date, end_date);
